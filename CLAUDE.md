...
```
Blockchain Event → Ponder Indexer → Postgres → GraphQL API → Frontend (urql + React Query)
                                                            ↓
...
**Indexer:**
- `indexer/src/index.ts` - Event handlers for all contract events
...
7. Add GraphQL query in `frontend/src/lib/graphql/queries.ts`
8. Create React Query hook in `frontend/src/hooks/queries/`
